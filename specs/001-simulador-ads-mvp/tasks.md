...
- [x] T001 [F1] Inicializar projeto Laravel via Docker (Sail) com MySQL
...
- [x] T002 [F1] Configurar `.env` com credenciais MySQL do Sail
- [x] T003 [P] [F1] Adicionar entradas ao `.gitignore` (node_modules, .env, vendor)

### Migrations

- [x] T004 [F2] Criar migration `create_segments_table`:
  - `id`, `name` (string, unique), `avg_ctr` (decimal 5,4), `avg_cpc` (decimal 8,2), `avg_conversion_rate` (decimal 5,4)
  - `technical_justification` (text/longText) ← **[NEW v2]**
...
- [x] T005 [P] [F2] Criar migration `create_regions_table`:
  ...
  - Índice único em `name`
- [x] T006 [F2] Adicionar campo `role` (enum: user/admin, default 'user') à tabela `users` via migration
- [x] T007 [F2] Criar migration `create_simulations_table`:
...
- [x] T008 [P] [F2] Criar Model `Segment` em `app/Models/Segment.php`:
  - fillable: name, avg_ctr, avg_cpc, avg_conversion_rate, technical_justification, confidence_score
  - Validação via accessor: `confidence_score` clampado a 0.0–1.0
- [x] T009 [P] [F2] Criar Model `Region` em `app/Models/Region.php`:
  - fillable: name, total_population, reachable_audience, avg_cpm, confidence_score
- [x] T010 [F2] Criar Model `Simulation` em `app/Models/Simulation.php`:
  - fillable + casts: results_json → array, campaign_month → integer
  - belongsTo: User, Segment, Region
- [x] T011 [F2] Atualizar Model `User`:
  - hasMany: Simulations
  - Accessor para `isAdmin`

### Seeders

- [x] T012 [P] [F2] Criar `SegmentSeeder` com 10+ segmentos:
  - Saúde, Imobiliário, E-commerce Moda, SaaS B2B, Educação Online, Alimentação, Automotivo, Finanças, Turismo, Fitness
  - Preencher `technical_justification` para cada segmento ← **[NEW v2]**
  - Cada segmento com `confidence_score = 0.7` ← **[NEW v2]**
- [x] T013 [P] [F2] Criar `RegionSeeder` com 27 estados brasileiros:
  - Dados de população IBGE, público alcançável (70-80%), CPM estimado
  - Cada região com `confidence_score = 0.7` ← **[NEW v2]**
- [x] T014 [F2] Registrar seeders no `DatabaseSeeder.php` + executar:
...
- [x] T015 [F3] Instalar e configurar Laravel Sanctum:
  - `./vendor/bin/sail composer require laravel/sanctum`
- [x] T016 [F3] Criar `RegisterController` com validação (name, email, password)
- [x] T017 [P] [F3] Criar `LoginController` com autenticação via Sanctum token
- [x] T018 [F3] Registrar rotas de auth em `routes/api.php`: POST /register, /login, /logout

### Endpoints da Aplicação

- [x] T019 [F3] Criar `ParameterController@index`:
  - `GET /api/parameters` → retorna segments (com confidence_score) + regions (com confidence_score) agrupados
- [x] T020 [F3] Criar `SimulationController@store`:
  - `POST /api/simulations` — recebe payload JSON incluindo `campaign_month` ← **[NEW v2]**
- [x] T021 [P] [F3] Criar `SimulationController@index`:
  - `GET /api/simulations` — histórico ordenado por data

### SimulationService (Clean Code)

- [x] T022 [F3] Criar `SimulationService` em `app/Services/`:
...
- [x] T023 [F3] Criar Middleware `IsAdmin` verificando `$request->user()->role === 'admin'`
- [x] T024 [F3] Criar `ImportController@importSegments`:
  - `POST /api/admin/import/segments` — valida colunas obrigatórias: name, avg_ctr, avg_cpc, avg_conversion_rate, **technical_justification**, **confidence_score** ← **[NEW v2]**
  - Validar `confidence_score` no range 0.0–1.0; rejeitar linhas fora do range
  - Upsert por name
- [x] T025 [P] [F3] Criar `ImportController@importRegions`:
...
- [x] T026 [F3] Criar `ImportService` em `app/Services/`:
  - Validação de colunas obrigatórias (incluindo technical_justification e confidence_score)
  - Validação de range para confidence_score
  - Parser CSV/Excel (Maatwebsite/Laravel-Excel ou parser nativo)
- [x] T027 [F3] Criar `DataHealthService` em `app/Services/` ← **[NEW v2]**:
...
- [x] T028 [F3] Criar `ImportController@dataHealth` ← **[NEW v2]**:
  - `GET /api/admin/data-health` — retorna status de saúde via DataHealthService
- [x] T029 [F3] Registrar todas as rotas admin em `routes/api.php` protegidas pelo middleware IsAdmin:
...
- [x] T030 [F4] Instalar Vue.js 3, Vue Router, Pinia via Sail:
  ```bash
  ./vendor/bin/sail npm install vue@3 vue-router@4 pinia
  ```
- [x] T031 [F4] Instalar e configurar Vite para Laravel:
  ```bash
  ./vendor/bin/sail npm install @vitejs/plugin-vue
  ```
  - Configurar `vite.config.js` com alias `@` para `resources/js`
- [x] T032 [F4] Criar ponto de entrada `resources/js/app.js` com createApp, use(router), use(pinia)
- [x] T033 [F4] Criar `resources/js/router/index.js` com rotas: `/login`, `/dashboard`, `/admin`, `/history`
...
- [x] T034 [F4] Instalar Tailwind CSS via Sail:
  ```bash
  ./vendor/bin/sail npm install -D tailwindcss postcss autoprefixer
  ./vendor/bin/sail npx tailwindcss init -p
  ```
- [x] T035 [F4] Configurar `tailwind.config.js` com TODOS os tokens do DESIGN.md:
...
- [x] T036 [F4] Criar `resources/js/assets/css/app.css` com:
...
- [x] T040 [P] [F4] Criar ícones PWA (192x192, 512x512) em `public/icons/`
- [x] T041 [F4] Verificar Service Worker cacheando assets e lógica JS
...
- [x] T042 [F4] Criar composable `useSeasonality` em `resources/js/composables/` ← **[NEW v2]**:
  - **Constantes**:
    ```js
...
- [x] T043 [F4] Criar composable `useCalculator` em `resources/js/composables/`:
...
- [x] T044 [F4] Criar `useParameterStore` em `resources/js/stores/`:
...
- [x] T045 [F4] Criar `useAuthStore` em `resources/js/stores/`:
  - State: `user: null`, `token: null`, `isAuthenticated: boolean`
  - Actions: `register()`, `login()`, `logout()`, `fetchUser()`

- [x] T046 [F4] Criar `useSimulationStore` em `resources/js/stores/`:
...
- [x] T047 [F4] Criar composable `useAlerts` em `resources/js/composables/`:
...
- [x] T048 [F4] **VALIDAÇÃO DE INTEGRAÇÃO**: Montar página temporária de teste que:
...
- [x] T049 [F5] Desenvolver `LoginForm.vue` baseado no `login.html`
...
- [x] T050 [F5] Montar layout principal da `LoginPage.vue`
...
- [x] T051 [P] [F5] Extrair `TopAppBar.vue` do `dashboard.html`
  - Logo, nav links, ícones, botão "Nova Simulação", avatar usuário
  - `bg-white/80 backdrop-blur-xl`
- [x] T052 [P] [F5] Extrair `AdminSidebar.vue` do `admin.html`
  - Logo + badge "Admin Console", nav links, footer
  - `bg-slate-50`, `w-64`, `fixed left-0`
- [x] T053 [P] [F5] Extrair `BottomNavBar.vue` do `admin.html`
...
- [x] T054 [F5] Extrair e vincular `ParameterPanel.vue`
...
- [x] T055 [F5] Construir `BudgetSummaryCard.vue` com cálculos de dedução
...
- [x] T056 [F5] Construir `ScenarioCard.vue` para exibir KPIs em três cenários
...
- [x] T057 [F5] Componentizar `AlertBanner.vue`
  - Props: `type` (warning|tip|season|insight), `title`, `description`
  - Tipo `season` com cor laranja para alertas de sazonalidade ← **[NEW v2]**
  - Tipo `insight` para exibir "Insight do Especialista" baseado na `technical_justification` do segmento selecionado ← **[NEW v2]**
  - Integrado com `useAlerts`
- [x] T058 [P] [F5] Adicionar `MetricGlassCard.vue` ao dashboard
  - Overlay gradiente, glass-metric card com "Alcance Estimado Total" reativo
- [x] T059 [F5] Componentizar os botões de ação (Exportar)
...
- [x] T060 [F5] Assemble total de `DashboardPage.vue`
...
- [x] T067 [F5.5] Desenvolver `HistoryPage.vue`
...
- [x] T070 [P] [F6] Validar uploads CSV no ImportService:
  - Tipo MIME (text/csv, application/vnd.ms-excel)
  - Tamanho máximo 5MB, extensão (.csv, .xlsx)
  - `confidence_score` no range 0.0–1.0 (rejeitar fora do range)
- [x] T071 [P] [F6] Rate limiting: `/api/login` (5/min), `/api/admin/import/*` (10/min)
- [x] T072 [F6] CSRF protection + sanitização em todos os FormRequests
- [x] T073 [F6] Revisar que rotas admin são inacessíveis sem middleware IsAdmin

### Auditoria PWA

- [x] T074 [F6] Lighthouse PWA audit (score ≥ 90)
  - Manifest, Service Worker, HTTPS, ícones
- [x] T075 [F6] Testar offline: desconectar → interface carrega → cálculos funcionam

### Build e Deploy

- [x] T076 [F6] Build produção: `./vendor/bin/sail npm run build`
- [x] T077 [F6] Verificar assets em `public/build/`
- [x] T078 [F6] Documentar script de deploy (CloudPanel):
  ```bash
  ...
  ```
- [x] T079 [F6] Criar usuário admin via seeder/tinker para produção
...
| Justificativa Técnica | Novo campo em segments e alertas com "Insight do Especialista" | T004, T008, T012, T024, T026, T057, T062 |
